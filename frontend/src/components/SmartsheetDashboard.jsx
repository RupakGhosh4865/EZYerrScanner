...
import { connectToSmartsheet, analyzeSheet, executeActions, downloadCorrectedCsv } from '../api/client'
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function SmartsheetDashboard() {
  ...
  const handleDownloadCsv = async () => {
    setLoading(true); setLoadingMsg('Preparing corrected CSV...'); setLoadingSub(''); setError(null)
    try {
      const blob = await downloadCorrectedCsv({
        sheetId: selectedSheet.id,
        approvedActionIds: [...approvedIds],
        proposedActions: analysis?.proposed_actions || [],
        columnMap: analysis?.column_map || {},
        sheetName: analysis?.sheet_name || selectedSheet?.name || 'sheet'
      })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      const safeName = (analysis?.sheet_name || selectedSheet?.name || 'sheet').replace(/\s+/g, '_')
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    <div className="smartsheet-dashboard-container">
      {/* Header */}
        ...
        {!loading && step === 4 && execution && (
          <div className="result-view" style={{ animation: 'slideUp 0.5s ease' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem' }}>
              <button className="approve-btn" onClick={handleDownloadCsv}>
                Download corrected CSV
              </button>
              <button style={{ background: 'transparent', border: '1px solid var(--border-neon)', color: 'var(--text-grey)', borderRadius: 8, padding: '0.75rem 1.25rem', cursor: 'pointer', fontSize: '0.8rem' }}
                onClick={handleReset}>
